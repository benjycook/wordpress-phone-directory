...
function sdir_edit_listing_category_field($taxonomy) {
	...
	echo '<tr class="form-field">
		<th scope="row" valign="top"><label for="taxonomy_image">' . __('Image', 'phone-dir') . '</label></th>
		...
		<button class="sdir_upload_image_button button">' . __('Upload/Add image', 'phone-dir') . '</button>
		<button class="sdir_remove_image_button button">' . __('Remove image', 'phone-dir') . '</button>
		</td>
...
function sdir_taxonomy_columns( $columns ) {
	...
	$new_columns['thumb'] = __('Image', 'phone-dir');
...
function sdir_taxonomy_column( $columns, $column, $id ) {
	if ( $column == 'thumb' )
		$columns = '<span><img src="' . sdir_taxonomy_image_url($id, 'thumbnail', TRUE) . '" alt="' . __('Thumbnail', 'phone-dir') . '" class="wp-post-image" /></span>';
